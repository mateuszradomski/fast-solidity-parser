...
contract MilestoneTracker {
    ...
    modifier onlyRecipient { if (msg.sender !=  recipient) revert Error(); _; }
    modifier onlyArbitrator { if (msg.sender != arbitrator) revert Error(); _; }
    modifier onlyDonor { if (msg.sender != donor) revert Error(); _; }
    ...
    modifier campaignNotCanceled { if (campaignCanceled) revert Error(); _; }
    modifier notChanging { if (changingMilestones) revert Error(); _; }
...
////////////
    ...
    function acceptProposedMilestones(bytes32 _hashProposals
        ...
        if (!changingMilestones) revert Error();
        if (keccak256(proposedMilestones) != _hashProposals) revert Error();
        ...
        if (!itmProposals.isList()) revert Error();
        ...
        while(itrProposals.hasNext()) {
            ...
            if (!itmProposal.isList()) revert Error();
    ...
    {
        if (_idMilestone >= milestones.length) revert Error();
        Milestone storage milestone = milestones[_idMilestone];
        if (  (msg.sender != milestone.milestoneLeadLink)
            &&(msg.sender != recipient))
            revert Error();
        if (milestone.status != MilestoneStatus.AcceptedAndInProgress) revert Error();
        if (block.timestamp < milestone.minCompletionDate) revert Error();
        if (block.timestamp > milestone.maxCompletionDate) revert Error();
        ...
        public campaignNotCanceled notChanging
    {
        ...
        if ((msg.sender != milestone.reviewer) ||
            (milestone.status != MilestoneStatus.Completed)) revert Error();
    ...
    function authorizePayment(uint _idMilestone) internal {
        ...
        if (milestone.status == MilestoneStatus.AuthorizedForPayment) revert Error();
